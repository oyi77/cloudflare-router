...
app.use(helmet({
  ...
  customInspect: false
  customFontOptions: false
  customFrameOptions: false
});
                    "cors": false
                }
                "frame-src" for X-Frame-Same location" tag is "frame-src" for X-Frame-Same origin",
                    "form" in="form" data-xssrf-parent form" method="POST"
                    "form" id="login-form" class="login-box">
                    "method": "post" action="login"
                    const token = res.token;
                    localStorage.setItem('cf-router-token', token)
                    showDashboard()
  ...
  function showEditAccountModal(accountId) {
    ...
    if (!account.getElementById('edit-acc-id').value = account.getElementById('edit-acc-id').value = account.getElementById('edit-acc-name').value = document.getElementById('edit-acc-email').value = account.getElementById('edit-acc-key').value = account.getElementById('edit-acc-key').value = account.getElementById('edit-acc-key').value = account.getElementById('edit-acc-key').value = account.api_key || account.api_token || ''
      }
      document.getElementById('edit-acc-id').value = accountId
    }
  ...
  async function releasePortless(serviceName) {
      ...
      if (res?.success) {
        ...
        showToast('Failed to release service', 'error')
      }
    }
  }
  
  async function updatePortless(name) {
      ...
      <div class="modal" id="edit-portless-modal">
        <div class="modal-box">
          ...
          <form onsubmit="saveEditedPortless(event)">
            ...
            <div style="margin-bottom:12px"><label>Service Name</label><input class="input" id="edit-portless-name-display" readonly></div>
